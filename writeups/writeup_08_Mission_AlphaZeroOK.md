...
- **Nom du fichier** : writeup_08_Mission_AlphaZeroOK.txt
- **Difficulté estimée** : Non spécifiée
- **Flag découvert** : `code{Vyb#Hq6y?e}`
...
l'Objectif : retrouver le flag de 16 caractères.
...
offset = 10499 + 14   #Calcul de l'offset des bloc après "Bad Password!" :
...
Pas besoin de tout creuser:

on voit quatre petites fonctions appelées après la lecture de l’entrée ;

la 1ère vérifie la longueur == 16,

les 3 autres manipulent un tampon (XOR / rotations) et comparent aux 3 blocs.

Hypothèse & test dans Python

On soupçonne que deux blocs servent de clé/chiffrement pour le troisième (ou pour l’entrée).

Après avoir extrait deux blocs de 16 octets chacun dans la section .data du binaire, on a appliqué un XOR entre ces deux blocs (bloc1 ⊕ bloc3).
Cette opération est réversible et permet ici de retrouver directement le flag caché.
En effet, l'étudiant ayant développé le crackme avait stocké une version chiffrée du flag à l'aide de cette méthode simple mais efficace.
